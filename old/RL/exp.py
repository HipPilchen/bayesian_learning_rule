...
######################################
## Define experiment class for VOGN ##
######################################

class ExperimentVOGNMLPBinClass(Experiment):
    ...
    def __init__(self, data_set, model_params, train_params, optim_params, evals_per_epoch=1, normalize_x=False, results_folder="./results", data_folder=DEFAULT_DATA_FOLDER, use_cuda=torch.cuda.is_available()):
        ...
        # Initialize model
        self.model = IndividualGradientMLP(input_size = self.data.num_features,
        ...
        # Define objective
        self.objective = metrics.avneg_loglik_bernoulli

        # Initialize optimizer
        self.optimizer = VOGN(self.model.parameters(),
    ...
    def _evaluate_model(self, metric_dict, x_train, y_train, x_test, y_test):
        ...
        # Store train metrics
        metric_dict['train_pred_logloss'].append(metrics.predictive_avneg_loglik_bernoulli(logits_list, y_train).detach().cpu().item())
        metric_dict['train_pred_accuracy'].append(metrics.sigmoid_predictive_accuracy(logits_list, y_train).detach().cpu().item())
        metric_dict['elbo_neg_ave'].append(metrics.avneg_elbo_bernoulli(logits_list, y_train, train_set_size = self.data.get_train_size(), kl = self.optimizer.kl_divergence()).detach().cpu().item())
        ...
        # Store test metrics
        metric_dict['test_pred_logloss'].append(metrics.predictive_avneg_loglik_bernoulli(logits_list, y_test).detach().cpu().item())
        metric_dict['test_pred_accuracy'].append(metrics.sigmoid_predictive_accuracy(logits_list, y_test).detach().cpu().item())
    ...
    def _closure(self, x, y):
        ...
        logits, H_list, Z_list =  self.model.forward(x, individual_grads=True)
