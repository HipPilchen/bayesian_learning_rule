...
class MyLinear_1st_order(object):
    ...
    def forward(self, x, sample = True, n_samples = 1):
        ...
        x_samples = self.weights * (self.x.reshape(1, -1)[np.newaxis, :, :])
        x_samples = np.sum(x_samples, axis = 2)
        return np.mean(x_samples + self.bias, axis = 0)
...
def plot_decision_boundary(ax, X, Y, classifier, x_min=-1.5, x_max=2.5, y_min=-1, y_max=1.5):
    ...
    Z = []
    for x in to_forward:
        Z.append(classifier.forward(x, n_samples=1000))
    Z = np.array(Z)
...
def plot_decision_boundary_comparison(ax, X, Y, classifier1, classifier2, x_min=-1.5, x_max=2.5, y_min=-1, y_max=1.5):
    ...
    for x in to_forward:
        Z1_plot_sample = []
        for _ in range(n_samples_plot):
            pred = classifier1.forward(x, n_samples=1)
            Z1_plot_sample.append(pred[0])  # Extraire la valeur du array(1,)
        Z1_plot.append(Z1_plot_sample)
        
        mean_pred = classifier1.forward(x, n_samples=n_samples_mean)
        Z1_mean.append(mean_pred[0])  # Extraire la valeur
        
        pred2 = classifier2.forward(x)
        Z2.append(pred2[0])  # Extraire la valeur

    # Conversion de Z1_plot en un array numpy pour un traitement plus facile
    Z1_plot = np.array(Z1_plot).reshape(xx.shape + (n_samples_plot,))
    Z1_mean = np.array(Z1_mean).reshape(xx.shape)
    Z2 = np.array(Z2).reshape(xx.shape)
    
    # Tracer le contour pour Z1_mean et Z2
    ...
    # Tracer les contours pour Z1_plot avec transparence
    for i in range(n_samples_plot):
        ax.contour(xx, yy, Z1_plot[:,:,i], levels=[0.5], colors='red', linestyles='solid', alpha=0.1)  # rouge transparent

    # Ajouter une légende
    lines = [plt.Line2D([0], [0], color='red', linestyle='solid'),
             plt.Line2D([0], [0], color='blue', linestyle='solid'),
             plt.Line2D([0], [0], color='red', linestyle='solid', alpha=0.1)]
    labels = ['Mean BLR', 'SGD', 'BLR Samples']
...
class Sequential(object):
    ...
    def forward(self, x, n_samples = 1):
        for layer in self.layers:
            x = layer.forward(x, n_samples = n_samples)
        return x
