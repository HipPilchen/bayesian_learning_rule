...
class MyVadam(Optimizer):
    ...
    def __init__(self, params, train_set_size, lr=1e-3, betas=(0.9, 0.999), prior_prec=1.0, prec_init=1.0, num_samples=1):
        ...
        super(MyVadam, self).__init__(params, defaults)
        ...
        for s in range(self.num_samples):
            ...
            for group in self.param_groups:
                for p in group['params']:
                    ...
                    # A noisy sample
                    raw_noise = torch.normal(mean=torch.zeros_like(p.data), std=0.01) # No noise as the noise is in the closure()
            ...
            loss = closure() # the noise is in this clusure fct
...
class MyVOGN(Optimizer):
    ...
    def step(self, closure):
        ...
        print("num mc samples : ", defaults['num_samples'])
        for _ in range(defaults['num_samples']):
            # Sample a parameter vector:
            raw_noise = torch.normal(mean=torch.zeros_like(mu), std=1.0)
            p = torch.addcdiv(mu, 1., raw_noise, torch.sqrt(Precision))
